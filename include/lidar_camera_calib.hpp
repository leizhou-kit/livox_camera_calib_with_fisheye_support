...
public:
  ...
  Calibration(const std::string &image_file, const std::string &pcd_file,
              const std::string &calib_config_file, const std::string &camera_type);
...
/////////this function deprecated.
  ...
  if (camera_type_ == "pinhole") {
        ...
        p1_ = 0.0;
        p2_ = 0.0;
        std::cout << "Camera Type: fisheye"<<std::endl;
        std::cout << "Camera Matrix: " << std::endl << camera_matrix_ << std::endl;
        std::cout << "Distortion Coeffs: " << std::endl << dist_coeffs_ << std::endl;
    }
...
void Calibration::colorCloud(
  ...
  if (camera_type_ == "pinhole") {
    distortion_coeff = (cv::Mat_<double>(1, 5) << k1_, k2_, p1_, p2_, k3_);
  } else if (camera_type_ == "fisheye") {
    distortion_coeff = (cv::Mat_<double>(1, 4) << k1_, k2_, k3_, k4_);
  }
  cv::Mat r_vec =
      (cv::Mat_<double>(3, 1)
...
void Calibration::buildVPnp(
  ...
  else{std::cout<<camera_type_<<"wrong type of camera! check config file"<<std::endl;}
