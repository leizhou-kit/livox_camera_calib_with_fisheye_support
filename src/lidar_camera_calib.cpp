...
class pnp_calib {
public:
  pnp_calib(PnPData p, const string& cam_type) : pd(p), camera_type(cam_type) {}
  ...
  bool operator()(const T *_q, const T *_t, T *residuals) const {
    ...
    T xd, yd;
    undistortPoint(xo, yo, distorT, xd, yd);
  ...
  static ceres::CostFunction *Create(PnPData p, const string& cam_type) {
    return (new ceres::AutoDiffCostFunction<pnp_calib, 2, 4, 3>(new pnp_calib(p, cam_type)));
  }

private:
  template <typename T>
  void undistortPoint(const T& xo, const T& yo, const Eigen::Matrix<T, 4, 1>& distorT, T& xd, T& yd) const {
    if (camera_type == "pinhole") {
      T r2 = xo * xo + yo * yo;
      T r4 = r2 * r2;
      T distortion = T(1.0) + distorT[0] * r2 + distorT[1] * r4;
      xd = xo * distortion + (distorT[2] * xo * yo + distorT[2] * xo * yo) +
          distorT[3] * (r2 + xo * xo + xo * xo);
      yd = yo * distortion + distorT[3] * xo * yo + distorT[3] * xo * yo +
          distorT[2] * (r2 + yo * yo + yo * yo);
    } else if (camera_type == "fisheye") {
      T r = sqrt(xo * xo + yo * yo);
      T theta = atan(r);
      T theta2 = theta * theta;
      T theta4 = theta2 * theta2;
      T theta6 = theta4 * theta2;
      T theta8 = theta4 * theta4;
      T thetad = theta * (T(1.0) + distorT[0] * theta2 + distorT[1] * theta4 + 
                          distorT[2] * theta6 + distorT[3] * theta8);
      T scale = (r == T(0)) ? T(1.0) : thetad / r;
      xd = xo * scale;
      yd = yo * scale;
    }
  }
...
class vpnp_calib {
public:
  vpnp_calib(VPnPData p, const string& cam_type) : pd(p), camera_type(cam_type) {}
  ...
  static ceres::CostFunction *Create(VPnPData p, const string& cam_type) {
    return (new ceres::AutoDiffCostFunction<vpnp_calib, 2, 4, 3>(new vpnp_calib(p, cam_type)));
  }
    ...
    for (int round = 0; round < 3; round++) {
      ...
      for (int iter = 0; iter < max_iter; iter++) {
        ...
        calibra.buildVPnp(test_params, match_dis, false,
                          ...
                          pnp_list);
        //std::cout<<"pnp list "<<sizeof(calibra.rgb_egde_cloud_)<<"  "<<sizeof(calibra.plane_line_cloud_)<<"  "<<sizeof(pnp_list)<<endl;
...
int main(int argc, char **argv) {
  ...
  nh.param<string>("camera/camera_type", camera_type, "pinhole");

  Calibration calibra(image_file, pcd_file, calib_config_file, camera_type);
  ...
  if (camera_type == "pinhole") {
    ...
    distor << calibra.k1_, calibra.k2_, calibra.k3_, calibra.k4_;
  }
  R = calibra.init_rotation_matrix_;
  T = calibra.init_translation_vector_;
  std::cout << "Initial rotation matrix:" << std::endl << calibra.init_rotation_matrix_ << std::endl;
  std::cout << "Initial translation:" << calibra.init_translation_vector_.transpose() << std::endl;
    ...
    for (int cnt = 0; cnt < 2; cnt++) {
      ...
      if (use_vpnp) {
        for (auto val : vpnp_list) {
          ceres::CostFunction *cost_function = vpnp_calib::Create(val, camera_type);
          problem.AddResidualBlock(cost_function, NULL, ext, ext + 4);
        }
      } else {
        for (auto val : pnp_list) {
          ceres::CostFunction *cost_function = pnp_calib::Create(val, camera_type);
          problem.AddResidualBlock(cost_function, NULL, ext, ext + 4);
        }
      }
  ...
  Eigen::Matrix3d adjust_rotation = init_rotation.inverse() * R;
  ...
  while (ros::ok()) {
    ...
    sensor_msgs::ImagePtr img_msg = cv_bridge::CvImage(std_msgs::Header(), "bgr8", calibra.image_).toImageMsg();
