...
class pnp_calib {
public:
  pnp_calib(PnPData p, const std::string& cam_type) : pd(p), camera_type(cam_type) {}
  template <typename T>
  ...
  static ceres::CostFunction *Create(PnPData p, const std::string& cam_type) {
...
class vpnp_calib {
public:
  vpnp_calib(VPnPData p, const std::string& cam_type) : pd(p), camera_type(cam_type) {}
  template <typename T>
  bool operator()(const T *_q, const T *_t, T *residuals) const {
    Eigen::Matrix<T, 3, 3> innerT = inner.cast<T>();
    Eigen::Matrix<T, 4, 1> distorT = distor.cast<T>();
  ...
  static ceres::CostFunction *Create(VPnPData p, const std::string& cam_type) {
...
int main(int argc, char **argv) {
  ...
  nh.param<std::string>("camera/camera_type", camera_type, "pinhole");
  ...
  for (size_t i = 0; i < data_num; i++) {
    ...
    Calibration single_calib(image_file, pcd_file, calib_config_file, camera_type);
    ...
    if (camera_type == "pinhole") {
      ...
      single_calib.k4_ = 0.0;
    } else if (camera_type == "fisheye") {
      single_calib.k1_ = dist_coeffs[0];
      single_calib.k2_ = dist_coeffs[1];
      single_calib.k3_ = dist_coeffs[2];
      single_calib.k4_ = dist_coeffs[3];
      single_calib.p1_ = 0.0;
      single_calib.p2_ = 0.0;
    }
  ...
  if (camera_type == "pinhole") {
    ...
    distor << calibs[0].k1_, calibs[0].k2_, calibs[0].k3_, calibs[0].k4_;
  }
    ...
    for (int cnt = 0; cnt < 2; cnt++) {
      ...
      for (size_t i = 0; i < data_num; i++) {
        for (auto val : vpnp_list_vect[i]) {
          ...
          cost_function = vpnp_calib::Create(val, camera_type);
